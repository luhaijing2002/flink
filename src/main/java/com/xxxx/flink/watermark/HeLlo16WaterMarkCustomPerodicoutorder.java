 ...
 * @version 1.0
 */
public class HeLlo16WaterMarkCustomPerodicoutorder {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStreamSink<Tuple3<String,String, Long>> tuple3DataStreamSink = source.map(line -> {
                ...
                .assignTimestampsAndWatermarks(new myWatermarkStrategy02())
...
//自定义水位线生成策略
class  myWatermarkStrategy02 implements WatermarkStrategy<Tuple3<String,String,Long>>{
    ...
    private class MyPeriodicWatermarkGenerator implements WatermarkGenerator<Tuple3<String,String,Long>>{
        ...
        public void onEvent(Tuple3<String, String, Long> tuple3, long eventTimestamp, WatermarkOutput output) {
            this.maxTs = Math.max(tuple3.f2, maxTs);
        }

        //周期性的进行调用(一个是周期性的调用)flink自动的调用
        @Override
        public void onPeriodicEmit(WatermarkOutput watermarkOutput) {
            System.out.println("WatermarkOutput=【" + (maxTs - 1L)+"】");
            //为什么要减-1，因为是左闭右开，如果
